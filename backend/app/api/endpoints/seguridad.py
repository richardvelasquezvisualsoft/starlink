...
def create_politica(politica_in: PoliticasSeguridadCreate, db: Session = Depends(get_db), admin: Usuario = Depends(get_admin_user)):
    ...
    new_policy = PoliticasSeguridad(
        ...
        minutos_validez_token_reset=politica_in.duracion_token_reset_minutos,
        minutos_inactividad_sesion=politica_in.timeout_inactividad_minutos,
        minutos_maximos_sesion=politica_in.duracion_maxima_sesion_horas * 60,
        historial_passwords=politica_in.cantidad_passwords_historial,
        requerir_mfa_reseller=politica_in.mfa_obligatorio_reseller,
        requerir_mfa_cliente=politica_in.mfa_obligatorio_cliente,
...
def update_politica(id: int, politica_in: PoliticasSeguridadUpdate, db: Session = Depends(get_db), admin: Usuario = Depends(get_admin_user)):
    ...
    if politica_in.duracion_token_reset_minutos is not None: policy.minutos_validez_token_reset = politica_in.duracion_token_reset_minutos
    if politica_in.timeout_inactividad_minutos is not None: policy.minutos_inactividad_sesion = politica_in.timeout_inactividad_minutos
    if politica_in.duracion_maxima_sesion_horas is not None: policy.minutos_maximos_sesion = politica_in.duracion_maxima_sesion_horas * 60
    if politica_in.cantidad_passwords_historial is not None: policy.historial_passwords = politica_in.cantidad_passwords_historial
    if politica_in.mfa_obligatorio_reseller is not None: policy.requerir_mfa_reseller = politica_in.mfa_obligatorio_reseller
    if politica_in.mfa_obligatorio_cliente is not None: policy.requerir_mfa_cliente = politica_in.mfa_obligatorio_cliente
