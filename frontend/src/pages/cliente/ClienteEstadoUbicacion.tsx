...
export const ClienteEstadoUbicacion: React.FC = () => {
      ...
      const customIcon = L.divIcon({
        ...
        html: `
          <div class="relative group cursor-pointer transition-all ${borderClass}">
            <div class="w-7 h-7 rounded-full ${isCritical ? 'bg-red-500/20' : isWarning ? 'bg-amber-500/20' : 'bg-emerald-500/20'} border-2 ${isCritical ? 'border-red-500' : isWarning ? 'border-amber-500' : 'border-emerald-500'} flex items-center justify-center shadow-lg backdrop-blur-md">
      ...
      const popupContent = `
        <div class="p-3 bg-st-bg border border-st-border rounded-xl text-white font-sans min-w-[220px]">
          <div class="flex items-center justify-between gap-2 mb-2 pb-2 border-b border-st-border">
            <span class="font-bold text-sm text-white">${item.nombre}</span>
            <span class="px-2 py-0.5 rounded text-[10px] font-bold ${isCritical ? 'bg-red-500/10 text-red-400 border border-red-500/20' : isWarning ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' : 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'}">
              ${item.estado_operativo}
            </span>
          </div>
          <div class="space-y-1 text-xs text-st-muted mb-2">
            <div><strong class="text-white">ID:</strong> ${item.device_id}</div>
            <div><strong class="text-white">Ubicación:</strong> ${item.distrito}</div>
            ${item.numero_linea ? `<div><strong class="text-white">Línea:</strong> ${item.numero_linea}</div>` : ''}
            ${item.plan_contratado ? `<div><strong class="text-white">Plan:</strong> ${item.plan_contratado}</div>` : ''}
            <div><strong class="text-white">Latencia:</strong> ${item.latencia_ms} ms</div>
            <div><strong class="text-white">Packet Loss:</strong> ${item.ping_drop_rate}%</div>
            <div><strong class="text-white">Geozona:</strong> <span class="${isFuera ? 'text-red-400 font-bold' : 'text-emerald-400'}">${item.geozona_estado}</span></div>
          </div>
        </div>
      `;
  ...
  }, [loading, error, filteredDispositivos, selectedId]);

  // Center map on selected device
  const handleSelectDevice = (item: DispositivoItem) => {
    setSelectedId(item.id);
    if (mapInstanceRef.current) {
      mapInstanceRef.current.flyTo([item.latitud, item.longitud], 14, { duration: 1.2 });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-white font-sans uppercase">ESTADO Y UBICACIÓN</h1>
          <p className="text-xs text-st-muted mt-0.5">Monitoreo geográfico y estado operativo de terminales.</p>
        </div>
        <button
          onClick={fetchData}
          disabled={loading}
      ...
      {/* Loading State */}
      {loading && !kpis && (
        <div className="flex h-96 items-center justify-center bg-st-surface border border-st-border rounded-xl">
      ...
      {error && (
        <div className="flex flex-col items-center justify-center p-8 bg-st-surface border border-red-500/20 rounded-xl text-center space-y-4">
          ...
          <div>
            <h3 className="text-lg font-semibold text-white">No se pudo cargar el estado y ubicación</h3>
      ...
      {!loading && kpis && (
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            <div className="bg-st-surface border border-st-border rounded-xl p-3.5 flex flex-col justify-between">
              <div className="text-[11px] font-semibold text-st-muted uppercase tracking-wider mb-1">Equipos Totales</div>
              <div className="text-xl font-bold text-white">{kpis.equipos_totales}</div>
              <div className="text-[10px] text-st-muted mt-1">Terminales contratadas</div>
            </div>

            <div className="bg-st-surface border border-st-border rounded-xl p-3.5 flex flex-col justify-between">
              <div className="text-[11px] font-semibold text-st-muted uppercase tracking-wider mb-1">Equipos Online</div>
              <div className="text-xl font-bold text-emerald-400 flex items-center gap-1.5">
                <Wifi className="w-4 h-4" />
                <span>{kpis.equipos_online}</span>
              </div>
              <div className="text-[10px] text-st-muted mt-1">Operativo y conectado</div>
            </div>

            <div className="bg-st-surface border border-st-border rounded-xl p-3.5 flex flex-col justify-between">
              <div className="text-[11px] font-semibold text-st-muted uppercase tracking-wider mb-1">Equipos con Alerta</div>
              <div className="text-xl font-bold text-amber-400 flex items-center gap-1.5">
                <AlertTriangle className="w-4 h-4" />
                <span>{kpis.equipos_con_alerta}</span>
              </div>
              <div className="text-[10px] text-st-muted mt-1">Incidencias o cortes</div>
            </div>

            <div className="bg-st-surface border border-st-border rounded-xl p-3.5 flex flex-col justify-between">
              <div className="text-[11px] font-semibold text-st-muted uppercase tracking-wider mb-1">Fuera de Geozona</div>
              <div className="text-xl font-bold text-red-400 flex items-center gap-1.5">
                <ShieldAlert className="w-4 h-4" />
                <span>{kpis.equipos_fuera_geozona}</span>
              </div>
              <div className="text-[10px] text-st-muted mt-1">Fuera del perímetro</div>
            </div>

            <div className="bg-st-surface border border-st-border rounded-xl p-3.5 flex flex-col justify-between">
              <div className="text-[11px] font-semibold text-st-muted uppercase tracking-wider mb-1">Latencia Promedio</div>
              <div className="text-xl font-bold text-cyan-400">{kpis.latencia_promedio_ms} ms</div>
              <div className="text-[10px] text-st-muted mt-1">Ping promedio</div>
            </div>

            <div className="bg-st-surface border border-st-border rounded-xl p-3.5 flex flex-col justify-between">
              <div className="text-[11px] font-semibold text-st-muted uppercase tracking-wider mb-1">Packet Loss</div>
              <div className="text-xl font-bold text-purple-400">{kpis.packet_loss_promedio_pct}%</div>
              <div className="text-[10px] text-st-muted mt-1">Pérdida de paquetes</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-[calc(100vh-16rem)] min-h-[600px]">
            ...
            <div className="lg:col-span-5 bg-st-surface border border-st-border rounded-xl flex flex-col h-full overflow-hidden">
              {/* Search & Filter Bar */}
              <div className="p-3.5 border-b border-st-border space-y-2.5 bg-st-bg/50">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-st-bg border border-st-border rounded-lg pl-9 pr-4 py-1.5 text-xs text-white focus:border-st-accent outline-none"
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div>
                    <select
                      ...
                      className="w-full bg-st-bg border border-st-border rounded-md px-2 py-1 text-[11px] text-st-muted focus:text-white outline-none cursor-pointer"
                    >
                ...
                <div className="flex items-center justify-between text-[11px] text-st-muted pt-1">
                  <span>Equipos encontrados: <strong className="text-white">{filteredDispositivos.length}</strong></span>
                  {(searchQuery || statusFilter !== 'ALL' || districtFilter !== 'ALL' || geofenceFilter !== 'ALL' || alertFilter !== 'ALL') && (
                    <button
                      ...
                      className="text-st-accent hover:underline text-[10px]"
                    >
                      Limpiar filtros
                    </button>
                  )}
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-2.5">
                {filteredDispositivos.length > 0 ? (
                  filteredDispositivos.map(item => {
                    ...
                    return (
                      <div
                        ...
                        className={`p-3 bg-st-bg border rounded-xl transition-all cursor-pointer ${
                          isSelected 
                            ? 'border-st-accent bg-st-accent/5 ring-1 ring-st-accent/50 shadow-md' 
                            : 'border-st-border hover:border-white/20'
                        }`}
                      >
                        {/* Title & Status */}
                        <div className="flex items-start justify-between gap-2 mb-1.5">
                          <div>
                            ...
                            <div className="text-[10px] text-st-muted">{item.device_id}</div>
                          </div>
                        ...
                        {/* Location */}
                        <div className="flex items-center justify-between text-xs text-st-muted mb-2">
                          <div className="flex items-center gap-1">
                            <Navigation className="w-3 h-3 text-st-accent" />
                            <span className="font-medium text-white">{item.distrito}</span>
                          </div>
                          {item.numero_linea && (
                            <span className="text-[10px] text-st-muted">{item.numero_linea}</span>
                          )}
                        </div>

                        {/* Metrics Grid */}
                        <div className="grid grid-cols-3 gap-2 pt-2 border-t border-st-border/50 text-center">
                          <div className="bg-st-surface/50 rounded-lg p-1.5 border border-st-border/30">
                            <div className="text-[9px] text-st-muted uppercase">Latencia</div>
                            <div className="text-xs text-cyan-300 font-bold">{item.latencia_ms} ms</div>
                          </div>

                          <div className="bg-st-surface/50 rounded-lg p-1.5 border border-st-border/30">
                            <div className="text-[9px] text-st-muted uppercase">Packet Loss</div>
                            <div className="text-xs text-purple-300 font-bold">{item.ping_drop_rate}%</div>
                          </div>

                          <div className="bg-st-surface/50 rounded-lg p-1.5 border border-st-border/30">
                            <div className="text-[9px] text-st-muted uppercase">Geozona</div>
                            <div className={`text-xs font-bold ${isFuera ? 'text-red-400' : 'text-emerald-400'}`}>
                  ...
                  <div className="text-center text-xs text-st-muted py-12">
            ...
            <div className="lg:col-span-7 bg-st-surface border border-st-border rounded-xl flex flex-col h-full overflow-hidden relative">
              {/* Map Header Toolbar */}
              <div className="p-3 border-b border-st-border flex items-center justify-between bg-st-bg/60 z-10">
                <div className="flex items-center gap-2">
                  <MapPin className="w-4 h-4 text-st-accent" />
                  <span className="font-bold text-xs text-white uppercase tracking-wider">Mapa Geográfico de Flota</span>
                </div>
              ...
              {selectedId && (
                <div className="p-3 border-t border-st-border bg-st-bg/90 backdrop-blur-md flex items-center justify-between gap-4 z-10">
                  {(() => {
                    ...
                    return (
                      <>
                        <div className="flex items-center gap-3">
                          <div className="p-2 bg-st-surface border border-st-border rounded-lg">
                            <Radio className="w-4 h-4 text-st-accent" />
                          </div>
                          <div>
                            <div className="text-xs font-bold text-white flex items-center gap-2">
                              <span>{sel.nombre}</span>
                              <span className="text-[10px] text-st-muted">({sel.device_id})</span>
                            </div>
                            <div className="text-[11px] text-st-muted flex items-center gap-3 mt-0.5">
