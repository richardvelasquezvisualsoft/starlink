  ...
  return (
    <div className="max-w-4xl py-6 px-4">
      ...
      <div className="mb-6 flex justify-between items-center">
        <div>
          <h1 className="text-[20px] font-bold text-client-text-primary tracking-tight">Cambiar Contraseña</h1>
          <p className="text-[13px] text-client-text-secondary mt-1 font-medium">Usa una contraseña fuerte para proteger tu acceso.</p>
        </div>
      </div>
      ...
      <div className="bg-client-bg-surface rounded-[12px] shadow-sm border border-client-border overflow-hidden relative">
        ...
        <div className="p-8">
          <div className="flex items-center gap-4 mb-8 border-b border-client-border pb-6">
            <div className="w-12 h-12 rounded-full bg-client-accent-soft flex items-center justify-center text-client-accent border border-client-accent/20">
              <Shield className="w-6 h-6" />
            </div>
            <div>
              <h2 className="text-[16px] font-bold text-client-text-primary">Seguridad de Acceso</h2>
              <p className="text-[13px] text-client-text-secondary mt-1">Configura una nueva credencial para tu cuenta.</p>
            </div>
          </div>
          ...
          <div className="max-w-xl mx-auto space-y-6">
            ...
            <div>
              <label className="block text-[12px] font-bold text-client-text-secondary mb-1.5 uppercase">Contraseña Actual</label>
              <div className="relative">
                <input 
                  ...
                  className="dark-input w-full px-4 py-3 bg-[#1e2024] border border-client-border rounded-[8px] text-[15px] text-white focus:outline-none focus:ring-1 focus:ring-client-primary focus:border-client-primary placeholder:text-gray-400 pr-12 font-medium"
                ...
                <button 
                  ...
                  className="absolute right-3 top-1/2 -translate-y-1/2 p-2 rounded-md hover:bg-white/10 text-gray-300 hover:text-white transition-colors focus:outline-none cursor-pointer flex items-center justify-center"
                  title={showCurrent ? "Ocultar contraseña" : "Ver contraseña"}
                >
                  {showCurrent ? <EyeOff className="w-5 h-5 text-cyan-400" /> : <Eye className="w-5 h-5 text-gray-300 hover:text-white" />}
                </button>
              </div>
            </div>

            <div>
              <label className="block text-[12px] font-bold text-client-text-secondary mb-1.5 uppercase">Nueva Contraseña</label>
              <div className="relative">
                <input 
                ...
                >
                  {showNew ? <EyeOff className="w-5 h-5 text-cyan-400" /> : <Eye className="w-5 h-5 text-gray-300 hover:text-white" />}
                </button>
              </div>
              {formData.nueva_password && (
                <p className={`text-xs mt-1.5 flex items-center gap-1.5 ${isMinLength ? 'text-emerald-400 font-semibold' : 'text-amber-400 font-medium'}`}>
            ...
            <div>
              <label className="block text-[12px] font-bold text-client-text-secondary mb-1.5 uppercase">Confirmar Nueva Contraseña</label>
              <div className="relative">
                <input 
                ...
                >
                  {showConfirm ? <EyeOff className="w-5 h-5 text-cyan-400" /> : <Eye className="w-5 h-5 text-gray-300 hover:text-white" />}
                </button>
              </div>
              {formData.confirmar_password && (
                <p className={`text-xs mt-1.5 flex items-center gap-1.5 ${isMatching ? 'text-emerald-400 font-semibold' : 'text-red-400 font-medium'}`}>
            ...
            <div className="pt-6 flex justify-end">
              <button 
                onClick={handleSave}
                disabled={loading}
                className="w-[200px] h-[40px] bg-client-primary hover:bg-client-primary-hover text-white rounded-[8px] text-[13px] font-bold flex items-center justify-center gap-2 transition-colors disabled:opacity-50 cursor-pointer"
              >
                {loading ? (
...
export default ClienteCambiarPassword;
